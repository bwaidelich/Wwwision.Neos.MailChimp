...
/**
 * Controller for the MailChimp TYPO3 Neos module
 */
class MailChimpController extends AbstractModuleController {
	...
	public function indexAction() {
		try {
			...
			$this->addFlashMessage('An error occurred while trying to fetch lists from MailChimp: "%s"', 'Error', Message::SEVERITY_ERROR, array($exception->getMessage()));
		}
	}
	...
	public function listAction($listId) {
		...
		try {
			...
			$this->addFlashMessage('An error occurred while trying to fetch members for list "%s" from MailChimp: "%s"', 'Error', Message::SEVERITY_ERROR, array($list['name'], $exception->getMessage()));
		}
	}
	...
	protected function fetchListById($listId) {
		try {
			...
			$this->addFlashMessage('The list with id "%s" does not exist', 'This list does not exist', Message::SEVERITY_WARNING, array($listId));
			$this->redirect('index');
		} catch (\Mailchimp_Error $exception) {
			$this->addFlashMessage('An error occurred while trying to fetch list with id "%s" from MailChimp: "%s"', 'Error', Message::SEVERITY_ERROR, array($listId, $exception->getMessage()));
			$this->redirect('index');
		}
	}
